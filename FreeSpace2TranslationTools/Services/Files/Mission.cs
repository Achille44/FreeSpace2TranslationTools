...
{
	internal class Mission(string content) : IFile
	{
		...
		private void ExtractShowSubtitleTextContentToMessages()
		{
			...
			foreach (Match match in messages)
			{
				...
				{
					int iteration = int.Parse(match.Groups[1].Value[autoGeneratedMessage.Length..]);
		...
		private void ConvertHardcodedHudTextToVariables()
		{
			...
			List<MissionVariable> hudVariables = [];
			List<HudText> hudTextList = [];
		...
		private void ConvertSpecialMessageSendersToVariables()
		{
			...
			List<MissionVariable> variableList = [];
			...
			foreach (Match message in messageMatches)
			{
				...
				foreach (Match specialSender in specialSenderMatches)
				{
					...
					{
						MissionVariable variable = Variables.FirstOrDefault(v => v.DefaultValue == specialSender.Groups[1].Value)
							?? new(GiveMeAVariableName(specialSender.Groups[1].Value), specialSender.Groups[1].Value);
		...
		private void ConvertEachSexpStringParameterToVariable()
		{
			...
			foreach (Match sexp in sexpMatches)
			{
				...
				foreach (Match name in nameMatches)
				{
					...
					{
						MissionVariable variable = Variables.FirstOrDefault(v => v.DefaultValue == name.Groups[1].Value)
							?? new(GiveMeAVariableName(name.Groups[1].Value), name.Groups[1].Value);
						variableList.Add(variable);
						AddVariableToListIfNotExisting(variable);
					}
				}
		...
		private void ConvertSexpParametersToVariables(IEnumerable<Match> matches)
		{
			...
			foreach (Match match in matches)
			{
				...
				{
					MissionVariable variable = Variables.FirstOrDefault(v => v.DefaultValue == match.Groups[2].Value)
						?? new(GiveMeAVariableName(match.Groups[2].Value), match.Groups[2].Value);
					variableList.Add(variable);
					AddVariableToListIfNotExisting(variable);
				}
			}
